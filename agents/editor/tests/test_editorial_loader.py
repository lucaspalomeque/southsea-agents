...
class TestEditorialLoaderReuse(unittest.TestCase):
    """Verifica que el Editor usa las mismas funciones que la del Writer."""

    def test_load_voice_delegates_to_writer(self):
        """load_voice del editor es la misma función que la del writer."""
        from agents.writer.tools.editorial_loader import load_voice as writer_load_voice
        self.assertIs(load_voice, writer_load_voice)

    def test_load_formats_delegates_to_writer(self):
        """load_formats del editor es la misma función que la del writer."""
        from agents.writer.tools.editorial_loader import load_formats as writer_load_formats
